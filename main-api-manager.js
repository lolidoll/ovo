...
"我不太同意...【表情包】困惑【/表情包】"`;
    ...
    collectConversationForApi: function(convId) {
        ...
        const userNicknameToUse = (conv.userNicknameForChar || (this.AppState.user && this.AppState.user.nickname) || '').trim();
        const charNicknameToUse = (conv.charNickname || '').trim();
        ...
        // 真名/网名区分（新增）
        systemPrompts.push(`角色真名：${charName || '未设置'}；用户真名：${userNameToUse || '未设置'}。`);
        systemPrompts.push(`网名是社交软件中的虚拟昵称，不等于真名。聊天时可自然应用。`);
        if (charNicknameToUse) {
            systemPrompts.push(`【角色网名】你当前的社交网名是：${charNicknameToUse}`);
        }
        if (userNicknameToUse) {
            systemPrompts.push(`【用户网名】对方当前社交网名是：${userNicknameToUse}`);
        }
        systemPrompts.push(`【网名变更能力】当你想主动修改自己的社交网名时，可以在回复中使用指令 [SET_CHAR_NICKNAME]新网名 。系统会自动保存并隐藏该指令。`);
        systemPrompts.push(`【网名变更规则】仅在语境自然时使用该指令；一次回复最多使用一次；指令后只写新网名本体，不要附加解释。`);
        ...
        // 强制AI读取用户名称
        if (userNameToUse) {
            systemPrompts.push(`正在手机另一端使用社交软件与你对话的用户的名字为"${userNameToUse}"。`);
        }
...
8. 用户在群里的昵称（网名）是"${groupNickname}"`;
        ...
        msgs.forEach((m, index) => {
            ...
            // 网名变更提示：作为系统上下文传给AI，确保AI知道谁改成了什么
            if (m.type === 'nickname_change') {
                out.push({ role: 'system', content: `[网名变更] ${messageContent}` });
                return;
            }
            ...
            if (m.type === 'sent') {
                ...
                // 语音消息、地理位置、视频通话消息根据sender字段判断角色
