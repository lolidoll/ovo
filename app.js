            ...
            modal.innerHTML = `
                <div style="background: white; border-radius: 12px; padding: 20px; min-width: 300px; max-width: 500px; box-shadow: 0 4px 20px rgba(0,0,0,0.2);">
                    ...
                    <div style="margin-top: 16px; display: flex; gap: 8px; justify-content: flex-end;">
                        <button onclick="this.closest('[style*=position]').remove();" style="padding: 8px 16px; border: 1px solid #ddd; border-radius: 6px; background: #fff; cursor: pointer; font-size: 14px;">取消</button>
...
记住：表情包是情绪表达工具，图片是视觉内容；处理时方式完全不同。`);
        ...
        function renderEmojiMgmtGroups() {
            ...
            AppState.emojiGroups.forEach((group, index) => {
                ...
                // 默认分组显示"（默认）"标签
                btn.textContent = index === 0 ? group.name + ' （默认）' : group.name;
                ...
                if (index > 0) {
                    ...
                    deleteBtn.style.cssText = 'width:18px;height:18px;border:none;background:transparent;cursor:pointer;color:#999;font-size:16px;padding:0;display:flex;align-items:center;justify-content:center;transition:all 0.2s;';
                ...
                <div class="emoji-mgmt-content" style="max-width:500px;max-height:90vh;overflow-y:auto;">
                    ...
                    <div style="padding:16px;">
                        ...
                        <div style="margin-bottom:20px;">
                            ...
                            <div id="char-worldbooks-list" style="border:1px solid #ddd;border-radius:4px;padding:8px;max-height:120px;overflow-y:auto;">
                                ${localWbs.map(w => `
                                    <label style="display:flex;align-items:center;padding:6px 0;cursor:pointer;font-size:13px;">
                                        <input type="checkbox" class="wb-checkbox" value="${w.id}" style="margin-right:8px;cursor:pointer;">
                                        <span>${w.name}</span>
                                    </label>
                                `).join('')}
                            </div>
