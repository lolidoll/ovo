...
=== 薯片机网站完整功能知识库 ===
【核心定位】薯片机是一个QQ风格的AI角色聊天网站，支持与多个AI角色对话、朋友圈互动、语音通话等丰富功能

【主要功能模块】
1. 聊天系统
  - 支持与多个AI角色进行对话，每个角色可独立设定性格、背景
  - 支持文字、语音消息、图片、表情包、位置分享
  - 消息支持长按操作：复制、删除、撤回、转发、翻译、收藏
  - 支持多选消息批量操作
  - 聊天背景可自定义，消息气泡颜色和字体颜色可调节（RGB+透明度）
  - 虚拟滚动优化，支持大量消息流畅显示

2. 角色设置系统
  - 可设置角色名称、备注、头像、角色设定描述
  - 支持为每个角色绑定专属用户人设（用户名称、人设内容）
  - 可绑定表情包分组和局部世界书
  - Token统计功能，实时显示当前对话Token数和消息数量
  - 对话总结功能：自动总结或手动生成对话摘要
  - 聊天背景图片设置
  - 消息气泡颜色自定义（角色消息和用户消息分别设置RGB+透明度）
  - 消息字体颜色自定义

3. API配置系统
  - 主API设置：端点、密钥、模型选择、温度、频率惩罚、存在惩罚、Top P等参数
  - 副API设置：独立的端点、密钥、模型，用于翻译、总结等辅助功能
  - 系统提示词管理：可创建多个提示词模板并切换使用
  - 支持AI时间感知功能
  - 图片生成API配置（支持OpenAI、Stability AI等）

4. 朋友圈功能
  - 发布朋友圈：支持纯文字、文字+图片、纯图片
  - 可选择好友分组可见范围
  - 点赞、评论、转发功能
  - AI角色会自动生成评论并回复用户
  - 支持角色主动发布朋友圈（手动或AI自动生成）
  - 自动生成朋友圈：可设置时间间隔和生成条数
  - 通知系统：查看角色在朋友圈的回复
  - 个人资料展示：头像、昵称、访客量、背景图

5. 语音通话系统
  - 支持与AI角色进行语音通话
  - 实时语音识别（STT）和语音合成（TTS）
  - 通话界面显示通话时长、角色头像
  - 支持挂断、静音等操作
  - 浮动窗口模式，可在通话时切换到其他页面
  - 支持MiniMax TTS语音合成
...
8. 世界书系统
  - 创建全局世界书和局部世界书
  - 世界书包含关键词和内容，在对话中自动触发
  - 可为特定角色绑定局部世界书
  - 支持世界书的启用/禁用

9. 用户人设管理
  - 创建多个用户人设（不同的用户名称和性格设定）
  - 为不同角色绑定不同的用户人设
  - 设置默认人设
  - 在角色设置中快速切换和应用人设
...
10. 心声系统
  - AI回复时自动提取角色的内心想法（心声）
  - 双击角色头像可查看角色的心声记录
  - 心声包含：好感度、好感度变化、心情、穿搭、动作、内心想法、坏心思等
  - 心声数据自动保存并可查看历史记录
...
- 长按消息可以看到更多操作选项
- 双击角色头像可以查看心声
- 在角色设置中可以详细配置每个角色的各项参数
- API设置中可以配置主API和副API，副API用于翻译、总结等辅助功能
- 朋友圈功能在侧边栏"朋友圈"菜单中
- 语音通话需要配置TTS服务才能使用

当用户询问任何关于薯片机网站的功能、使用方法、配置说明时，你都可以基于以上知识库给出准确、详细的回答ᕱ⑅ᕱ`,
